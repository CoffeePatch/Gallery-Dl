...
const { NEW_RAW_DATA_DIR, RAW_DATA_DIR, MEDIA_DIR: OUTPUT_DIR } = require('../lib/paths');
...
// Parse CLI input overrides (--file / --dir)
const fileArgIndex = args.findIndex(arg => arg === '--file' || arg === '-f');
const dirArgIndex = args.findIndex(arg => arg === '--dir' || arg === '-d');

let SINGLE_FILE = null;
let DIRECTORY = NEW_RAW_DATA_DIR;

if (fileArgIndex !== -1 && args[fileArgIndex + 1]) {
    SINGLE_FILE = path.resolve(args[fileArgIndex + 1]);
    DIRECTORY = path.dirname(SINGLE_FILE);
} else if (dirArgIndex !== -1 && args[dirArgIndex + 1]) {
    DIRECTORY = path.resolve(args[dirArgIndex + 1]);
} else if (!fs.existsSync(DIRECTORY) || fs.readdirSync(DIRECTORY).filter(f => f.endsWith('.json')).length === 0) {
...
async function processFile(filePath, workerId) {
    ...
    const accountName = fileName.replace('_tweets.json', '').replace('.json', '');
    ...
    for (let i = 0; i < mediaRecords.length; i++) {
        ...
        const localFileName = constructFilename(mediaUrl, tweetId, dateStr, accountName);
...
async function startWorkerPool() {
    ...
    if (SINGLE_FILE) {
        if (!fs.existsSync(SINGLE_FILE)) {
            console.error(`Error: Specified file '${SINGLE_FILE}' does not exist.`);
            process.exit(1);
        }
        files = [path.basename(SINGLE_FILE)];
        DIRECTORY = path.dirname(SINGLE_FILE);
    } else {
