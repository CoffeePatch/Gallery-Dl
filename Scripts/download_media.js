...
const DIRECTORY = path.join(__dirname, '..', 'TweetData', 'RawData');
...
async function processFile(filePath, workerId) {
    ...
    // Filter media records (Type 3 usually represents media, or type: "photo"/"video")
    const mediaRecords = records.filter(record => 
        (Array.isArray(record) && record[0] === 3) || 
    ...
    for (let i = 0; i < mediaRecords.length; i++) {
        ...
        if (Array.isArray(record)) {
            // Assume gallery-dl format [3, tweetId, mediaData]
            tweetId = record[1];
            const mediaData = record[2];
            mediaUrl = mediaData.url || mediaData.src;
        } else {
            // Object format
        ...
        // Construct filename incorporating tweet ID for tracking
        const localFileName = `${tweetId}_${path.basename(cleanUrl, ext)}${ext}`;
        ...
        // Brief delay between downloads for rate-limiting
        await sleep(300);
    }
